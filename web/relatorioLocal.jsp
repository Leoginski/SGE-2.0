...
<html>
    ...
    <main>
        <form action="RelatorioLocalController?acao=exibirRelatorio" method="post" name="frmRelatorioLocal">
        <div class="input-field row">
            <select name="capacidadeLocal" <c:if test="${operacao == 'Excluir'}"> disabled</c:if>>
                <option value="" <c:if test="${local.capacidade == null}"> selected</c:if>></option>
                <c:forEach items="${locais}" var="local">
                    <option value="${local.capacidade}">
                    ${local.capacidade}
                    </option>  
                </c:forEach>
                        </select>
