...
<html>
    <head>
        <title>Relatorio Noticias</title>
        <!--jQuery-->
    ...
    <main>
        <form action="RelatorioPropostaNomesController?acao=exibirRelatorio" method="post" name="frmRelatorioProposta">
        <div class="input-field row">
            <select name="nomeProponente" <c:if test="${operacao == 'Excluir'}"> disabled</c:if>>
                <option value="" <c:if test="${proposta.proponente == null}"> selected</c:if>></option>
                <c:forEach items="${propostas}" var="proposta">
                    <option value="${proposta.proponente}">
                    ${proposta.proponente}
                    </option>  
                </c:forEach>
            </select>
